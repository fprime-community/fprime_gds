# GdsBridge Application

This deployment was auto-generated by the F' utility tool.

## Building and Running the GdsBridge Application

In order to build the GdsBridge application, or any other F´ application, we first need to generate a build directory. This can be done with the following commands:
...
The next step is to build the GdsBridge application's code.
```
fprime-util build
```
...
To run the ground system without starting the GdsBridge app:
```
...
```
cd GdsBridge/build-artifacts/<platform>/bin/
./GdsBridge -a 127.0.0.1 -p 50000
```
