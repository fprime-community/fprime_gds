...
void Test_FPRIME_GDS_Main(void)
{
    ...
    /*
     * FPRIME_GDS_Main does not return a value,
     ...
     * The first is for FPRIME_GDS_Init().  As this is in the same
     ...
     * the FPRIME_GDS_Init() failure path was taken.
     */
    FPRIME_GDS_Main();
     ...
     */
    UtAssert_UINT32_EQ(FPRIME_GDS_Data.RunStatus, CFE_ES_RunStatus_APP_ERROR);
    ...
    UT_CHECKEVENT_SETUP(&EventTest, FPRIME_GDS_PIPE_ERR_EID, "SAMPLE APP: SB Pipe Read Error, App Will Exit");
...
void Test_FPRIME_GDS_Init(void)
{
    /*
     * Test Case For:
    ...
    UtAssert_INT32_EQ(FPRIME_GDS_Init(), CFE_SUCCESS);
     ...
     */
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UtAssert_INT32_EQ(FPRIME_GDS_Init(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_STUB_COUNT(CFE_ES_WriteToSysLog, 1);

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UtAssert_INT32_EQ(FPRIME_GDS_Init(), CFE_SB_BAD_ARGUMENT);
    ...
    UtAssert_INT32_EQ(FPRIME_GDS_Init(), CFE_TBL_ERR_INVALID_OPTIONS);
...
{
    ADD_TEST(FPRIME_GDS_Main);
    ADD_TEST(FPRIME_GDS_Init);
}
