...
#include "fprime_gds_coveragetest_common.h"
...
void Test_FPRIME_GDS_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void FPRIME_GDS_ReportHousekeeping( const CFE_SB_CmdHdr_t *Msg )
     */
    ...
    FPRIME_GDS_SendHkCmd(NULL);

    /* Confirm message sent*/
    UtAssert_STUB_COUNT(CFE_SB_TransmitMsg, 1);
    UtAssert_ADDRESS_EQ(MsgSend, &FPRIME_GDS_Data.HkTlm);

    /* Confirm timestamp msg address */
    UtAssert_STUB_COUNT(CFE_SB_TimeStampMsg, 1);
    UtAssert_ADDRESS_EQ(MsgTimestamp, &FPRIME_GDS_Data.HkTlm);
...
void Test_FPRIME_GDS_NoopCmd(void)
{
    /*
     * Test Case For:
     * void FPRIME_GDS_NoopCmd( const FPRIME_GDS_Noop_t *Msg )
     */
    FPRIME_GDS_NoopCmd_t TestMsg;
    UT_CheckEvent_t      EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    /* test dispatch of NOOP */
    UT_CHECKEVENT_SETUP(&EventTest, FPRIME_GDS_NOOP_INF_EID, NULL);

    UtAssert_INT32_EQ(FPRIME_GDS_NoopCmd(&TestMsg), CFE_SUCCESS);
...
void Test_FPRIME_GDS_ResetCountersCmd(void)
{
    /*
     * Test Case For:
     * void FPRIME_GDS_ResetCounters( const FPRIME_GDS_ResetCounters_t *Msg )
     */
    FPRIME_GDS_ResetCountersCmd_t TestMsg;
    UT_CheckEvent_t               EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CHECKEVENT_SETUP(&EventTest, FPRIME_GDS_RESET_INF_EID, "SAMPLE: RESET command");

    UtAssert_INT32_EQ(FPRIME_GDS_ResetCountersCmd(&TestMsg), CFE_SUCCESS);
...
void Test_FPRIME_GDS_ProcessCmd(void)
{
    /*
     * Test Case For:
     * void  FPRIME_GDS_ProcessCmd( const FPRIME_GDS_ProcessCmd_t *Msg )
     */
    FPRIME_GDS_ProcessCmd_t   TestMsg;
    FPRIME_GDS_ExampleTable_t TestTblData;
    void *                    TblPtr = &TestTblData;

    memset(&TestTblData, 0, sizeof(TestTblData));
    memset(&TestMsg, 0, sizeof(TestMsg));

    /* Provide some table data for the FPRIME_GDS_Process() function to use */
    TestTblData.Int1 = 40;
    TestTblData.Int2 = 50;
    UT_SetDataBuffer(UT_KEY(CFE_TBL_GetAddress), &TblPtr, sizeof(TblPtr), false);
    UtAssert_INT32_EQ(FPRIME_GDS_ProcessCmd(&TestMsg), CFE_SUCCESS);
     ...
     * FPRIME_GDS_GetCrc() function is a stub.
     */
    ...
    UtAssert_INT32_EQ(FPRIME_GDS_ProcessCmd(&TestMsg), CFE_TBL_ERR_UNREGISTERED);
    ...
    UtAssert_INT32_EQ(FPRIME_GDS_ProcessCmd(&TestMsg), CFE_TBL_ERR_NO_ACCESS);
    UtAssert_STUB_COUNT(CFE_ES_WriteToSysLog, 4);
}

void Test_FPRIME_GDS_DisplayParamCmd(void)
{
    /*
     * Test Case For:
     * void  FPRIME_GDS_DisplayParamCmd( const FPRIME_GDS_DisplayParamCmd_t *Msg )
     */
    FPRIME_GDS_DisplayParamCmd_t TestMsg;
    UT_CheckEvent_t              EventTest;

    memset(&TestMsg, 0, sizeof(TestMsg));

    UT_CHECKEVENT_SETUP(&EventTest, FPRIME_GDS_VALUE_INF_EID, "FPRIME_GDS: ValU32=%lu, ValI16=%d, ValStr=%s");
    TestMsg.Payload.ValU32 = 10;
    TestMsg.Payload.ValI16 = -4;
    snprintf(TestMsg.Payload.ValStr, sizeof(TestMsg.Payload.ValStr), "Hello");

    UtAssert_INT32_EQ(FPRIME_GDS_DisplayParamCmd(&TestMsg), CFE_SUCCESS);
    /*
     * Confirm that the event was generated
     */
    UtAssert_UINT32_EQ(EventTest.MatchCount, 1);
...
{
    ADD_TEST(FPRIME_GDS_ReportHousekeeping);
    ADD_TEST(FPRIME_GDS_NoopCmd);
    ADD_TEST(FPRIME_GDS_ResetCountersCmd);
    ADD_TEST(FPRIME_GDS_ProcessCmd);
    ADD_TEST(FPRIME_GDS_DisplayParamCmd);
}
